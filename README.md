...
\(GDP_o\), for the income, expenditure, and output approaches to
measuring GDP, we might like to optimally combine them to get a series
\(GDP_c\) that reflects the measurement efforts of multiple scholars.
...
We stack our GDP measures into a vector \(Y_t\) measured over time
periods \(t\) to \(T\). We assume that our measurements of GDP are
...
block-diagonal in the signals, so that the 4 \(\times\) 4 matrix
\(\Sigma\) has zeros in rows 2:4 of column 1, and in columns 2:4 of row
1.
